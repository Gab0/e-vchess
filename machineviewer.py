...
class Application(Frame):
    ...
    def show_machine(self):
        ...
        for VW in range(len(self.paramVIEWER)):
            ...
            if self.machines[self.N].PARAMETERS[VW].locked:
                self.paramVIEWER[VW][1]['bg'] = 'red'
    ...
    def scrollbestmachines(self, ceiling, direction):
        if ceiling:
            ceiling = self.machines[self.N].ELO
        else:
            if direction >= 0:
                ceiling = +66666
            else:
    ...
    '''def show_attr_dump(self, attribute):
        ...
        for L in range(1, len(DUMP)):
            ...
            Label(text=DUMP[L][1] + " | " + str(round(int(DUMP[L][1]) / (int(DUMP[L][4]) + 1)
                                                      * 100, 3)) + "%", fg="green").grid(in_=self.VIEWDUMP, column=1, row=L)
            Label(text=DUMP[L][2] + " | " + str(round(int(DUMP[L][2]) / (int(DUMP[L][4]) + 1)
                                                      * 100, 3)) + "%", fg="grey").grid(in_=self.VIEWDUMP, column=2, row=L)
            Label(text=DUMP[L][3] + " | " + str(round(int(DUMP[L][3]) / (int(DUMP[L][4]) + 1)
    ...
    def createWidgets(self):
        ...
        self.clearmenu.add_command(
            label="PURGE machines", command=self.TOpurge)
        self.clearmenu.add_command(
            label="RELEASE orphans", command=lambda: ReleaseOrphan(self.DIR))
        self.clearmenu.add_command(
        ...
        self.popmenu.add_command(
            label="DEL the Worst", command=lambda: self.TOdeltheworst(1))
        self.popmenu.add_command(label="DEL 32 Worst",
                                 command=lambda: self.TOdeltheworst(32))
        self.popmenu.add_command(
            label="CLONE the Best", command=self.TOclonethebest)
        self.popmenu.add_separator()
        ...
        self.popmenu.add_command(label="Populate 256",
                                 command=lambda: self.TOpopulate(256))
        self.popmenu.add_separator()
        self.popmenu.add_command(
            label="Create 4 Hybrid", command=lambda: self.TOcreatehybrid(4))
        self.popmenu.add_command(
            label="Create 16 Hybrids", command=lambda: self.TOcreatehybrid(16))
        self.popmenu.add_separator()
        self.popmenu.add_command(
            label="Create 16 from Template", command=lambda: self.TOpopulatetemplate(16))
        self.popmenu.add_separator()
        self.popmenu.add_command(
            label="Routine Procedure", command=self.TOroutineprocedures)
        self.popmenu.add_command(
            label="send best to TOP", command=self.TOautotop)
        self.popmenu.add_separator()
        self.popmenu.add_command(
        ...
        self.actionmenu.add_command(
            label="Switch to Statlock Cycle.", command=self.TOswitchstatlock)
...
# actions called by upper menu.
    ...
    def TOLaunchMachine(self):
        ...
        LaunchEngineArgs = settings.engineARGS + \
            ['--specific', self.machines[self.N].filename]
    ...
    def renew_VIEWDUMP_canvas(self, alreadyexists):
        ...
        self.VIEWDUMP = Canvas(self, width=1000, height=500,
